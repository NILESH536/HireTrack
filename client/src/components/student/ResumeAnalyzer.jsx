    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-premium p-6">
      ...
      {a && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="space-y-5">
          ...
          {a.explanation && (a.explanation.reasoningSummary || a.explanation.recommendations?.length > 0) && (
            <Section title="Deep AI Diagnosis" icon={FiZap} defaultOpen={true}>
              <div className="space-y-4">
                {a.explanation.reasoningSummary && (
                  <div className="p-4 rounded-xl bg-electric/5 border border-electric/20 text-sm text-gray-300 leading-relaxed">
                    <strong className="text-electric block mb-1">AI Reasoning</strong>
                    {a.explanation.reasoningSummary}
                  </div>
                )}
                {a.explanation.recommendations?.length > 0 && (
                  <div>
                    <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Step-by-Step Action Plan</h4>
                    <ul className="space-y-2">
                      {a.explanation.recommendations.map((rec, i) => (
